...
parser.add_argument('--csv_path', type=str,default = '/raid/Aditya/Recon/Tumor/Recon/modl2D/CT1_EROTC_2D_TEST_ALL_TIME_NAS.csv')
...
model_path = dir+'CT1_2D_dcdw/R'+str(R_in)+'/'
if R_in == 2:
    subDirectory='11May_1106am_5L_10K_50E_AG'
elif R_in == 4:
    subDirectory='11May_0337pm_5L_10K_100E_AG'
elif R_in == 6:
    subDirectory='11May_1044pm_5L_10K_100E_AG'
elif R_in == 8:
    subDirectory='12May_0554am_5L_10K_100E_AG'
elif R_in == 10:
    subDirectory='12May_1242pm_5L_10K_100E_AG'   
elif R_in == 15:
    subDirectory='12Jul_1146am_5L_10K_100E_AG'
...
def save_nifti(recon,atb,i,aff,f):
    ...
    if 'CT1_2D.nii.gz' in f:
        f = f.replace('/CT1_2D.nii.gz','')
        f_dir = f + '/recon/R' + str(R_in)
        if not os.path.exists(f_dir):
            os.makedirs(f_dir)
        f_save = f_dir + '/CT1_2D_recon.nii.gz'
        nft_img = nib.Nifti1Image(recon, aff)
        nib.save(nft_img,f_save)
        atb = np.abs(atb)
        atb = atb.transpose((1,2,0))
        atb.astype(np.int16)
        f_save = f_dir + '/CT1_2D_us.nii.gz'
