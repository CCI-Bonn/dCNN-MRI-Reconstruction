...
parser.add_argument('--csv_path', type=str,default = '/raid/Aditya/Recon/Tumor/Recon/modl3D_april23/CT1_EROTC_3D_TEST_ALL_TIME_CPU_NAS.csv')
...
if R_in == 2:
    subDirectory='26Apr_0313pm_5L_5K_200E_AG'
elif R_in == 4:
    subDirectory='26Apr_1121pm_5L_5K_200E_AG'
elif R_in == 6:
    subDirectory='27Apr_0728am_5L_5K_200E_AG'
elif R_in == 8:
    subDirectory='27Apr_0331pm_5L_5K_200E_AG'
elif R_in == 10:
    subDirectory='28Apr_1225am_5L_5K_200E_AG'
elif R_in == 15:
    subDirectory='20Jul_0453pm_5L_5K_200E_AG'
...
print(f"Lambda value is {np.abs(lam_value)}")
...
def save_nifti(recon,atb,i,aff,f):
    ...
    if 'CT1_3D.nii.gz' in f:
        f = f.replace('/CT1_3D.nii.gz','')
        f_dir = f + '/recon/R' + str(R_in)
        if not os.path.exists(f_dir):
            os.makedirs(f_dir)
        f_save = f_dir + '/CT1_3D_recon.nii.gz'
        nft_img = nib.Nifti1Image(recon, aff)
        nib.save(nft_img,f_save)
        atb = np.abs(atb)
        # atb = atb.transpose((1,2,0))
        atb.astype(np.int16)
        f_save = f_dir + '/CT1_3D_us.nii.gz'
        nft_img = nib.Nifti1Image(atb, aff)
    elif 'CT1.nii.gz' in f:
        f = f.replace('/CT1.nii.gz','')
        f_dir = f + '/recon/R' + str(R_in)
        if not os.path.exists(f_dir):
            os.makedirs(f_dir)
        f_save = f_dir + '/CT1_recon.nii.gz'
        nft_img = nib.Nifti1Image(recon, aff)
        nib.save(nft_img,f_save)
        atb = np.abs(atb)
        # atb = atb.transpose((1,2,0))
        atb.astype(np.int16)
        f_save = f_dir + '/CT1_us.nii.gz'
        nft_img = nib.Nifti1Image(atb, aff)
    nib.save(nft_img,f_save)
...
# tmp = "/raid/Aditya/Recon/Tumor/EORTC_26101/213_355/20140620/T1.nii.gz"
# print(np.where(tstFilenames == tmp))
# tstFilenames[460]
...
#    new_saver = tf.train.import_meta_graph(modelDir+'/modelTst.meta')
    ...
    # for i in tqdm(range(350,nsamples)):
        # print(i)
